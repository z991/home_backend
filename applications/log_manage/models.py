...
from home_backend.config import LOG_TYPE_CHOICES, TYPE_LOGIN, MODULES_MAP, ACTION_MAP, TYPE_LOGOUT
...
class OperateLog(models.Model):
    ...
    def create_log(cls, request):
        ...
        print('action_module', action_module, type(action_module))
        if request.user.is_authenticated:
            logger = OperateLog(
                ip=OperateLog.get_remote_ip(request),
                user=request.user,
                action=ACTION_MAP[request.method],
                operationmodule=MODULES_MAP.get(str(action_module).split('.')[-1], 'None'),
